...
1. `U1` открывает управляемую ссылку и отправляет запрос на вступление в
   `@bbbikefan`; до одобрения Telegram не пускает его в группу.
2. По кнопке из сообщения бота открывает Mini App и подаёт заявку с именем,
   городом и комментарием о пользе компании.
3. Проверяет, что телефон не спрашивается и задания пока закрыты.
4. `S1` находит заявку поиском и одобряет её; durable outbox одобряет Telegram
   join request.
5. `U1` фактически вступает в группу, получает одно короткое приветствие и
   уведомление о решении.

PASS: один join request, одна заявка и одно решение; доступ и групповое
приветствие появляются только после одобрения.
