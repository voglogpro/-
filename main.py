...
BUILD_VERSION = "2026-07-26 · БибиЗадачи v2.0.0 (посты командами, приветствие, команды участников)"
...
@dp.message(F.text.regexp(r"^/(инструкция|instruction)"))
...
@dp.message(F.text.regexp(r"^/(ранг|rank|уровень)"))
...
async def _publish(message, parts, topic, kind):
    """Публикует пост в нужную подтему. Кнопки вешаем на последнее сообщение."""
...
# ── Команды участников ────────────────────────────────────────
@dp.message(F.text.regexp(r"^/(бонусы|баланс|balance)"))
...
@dp.message(F.text.regexp(r"^/(профиль|profile)"))
...
@dp.message(F.text.regexp(r"^/(задания|tasks)"))
...
@dp.message(F.text.regexp(r"^/(команды|help|старт)"))
