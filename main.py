...
BUILD_VERSION = "2026-07-25 · БибиЗадачи v1.8.0 (подписка на канал)"
...
async def post_instruction(message: Message):
    ...
    if message.chat.type == "private":
        if not GROUP_ID and not GROUP_USERNAME:
            ...
            return
        target_chat = GROUP_ID or f"@{GROUP_USERNAME}"
    try:
        await bot.send_message(
            target_chat, WORK_INSTRUCTION,
            message_thread_id=thread,
            reply_markup=_open_app_kb(),
            parse_mode="HTML",
            disable_web_page_preview=True)
        if message.chat.type == "private":
            await message.answer("Инструкция отправлена в подтему «Работа» ✅")
    except Exception as e:
