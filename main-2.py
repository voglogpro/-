...
GROUP_ID = -1003895375312   # ТЕСТ: группа t.me/c/3895375312 → -100 + номер
...
NPB_THREAD_ID = 1890       # ТЕСТ: тема «Npb»
...
# ТЕСТ: пока пусто — пришли @username тестового бота, и кнопка включится.
...
if not BOT_TOKEN:
    print(
        "КРИТИЧЕСКАЯ ОШИБКА: не задана переменная окружения BOT_TOKEN. "
        "Открой на карточке бота кнопку с колбой (переменные окружения), "
        "добавь BOT_TOKEN со значением токена и перезапусти.",
        flush=True,
    )
    logger.error("BOT_TOKEN не задан — выхожу.")
    sys.exit(1)
